class StatsManager {
  ...
  showStatsVisualization() {
    ...
    yLabel.textContent = "Energy/Flow";
    ...
    validStats.forEach((stat) => {
      try {
        // Energy point
        const energyPoint = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "circle"
        );
        const energyX = getTimeX(stat.timestamp);
        const energyY =
          height - (padding + (height - 2 * padding) * stat.energy);
        ...
        // Flow point
        const flowPoint = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "circle"
        );
